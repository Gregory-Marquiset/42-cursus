...
/*   Updated: 2025/01/17 03:53:04 by gmarquis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	st_cam_key(int keycode, t_cub *cub)
{
	if (keycode == 112)
	{
		cub->cam = !cub->cam;
		if (cub->cam == 1)
		{
			mlx_hook(cub->win_ptr, 6, 1L << 6, &cb_mouse_move, cub);
			mlx_mouse_hide(cub->mlx_ptr, cub->win_ptr);
		}
		else
		{
			mlx_hook(cub->win_ptr, 6, 1L << 6, NULL, cub);
