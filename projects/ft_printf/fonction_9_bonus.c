...
/*   Updated: 2023/12/21 01:56:59 by gmarquis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	zero_digit_dot(va_list args, char *str, int n)
{
	...
	if (*str == 'p')
		return (zero_digit_dot_p(va_arg(args, unsigned long long), n));
	if (*str == 'x')
...
int	zero_digit_dot_digit(va_list args, char *str, int n1)
{
	...
	if (*str == 'p')
		return (zero_digit_dot_digit_p(va_arg(args, unsigned long long), n1,
				n2));
	if (*str == 'x')
